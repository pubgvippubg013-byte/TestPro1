...
-- ========================================================
-- FIX GIFT MEGA SEED: Lấy trực tiếp Instance/UUID trong Inventory
-- ========================================================

local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

-- Bảng ánh xạ ID chuẩn cho Mega
local EXTRA_KEY_MAP = {
    ["Mega"] = "Mega",
    ["Mega Seed"] = "Mega"
}

-- Hàm bổ trợ quét tìm đúng Vật phẩm trong Inventory/Backpack
local function GetActualMegaItems()
    local foundItems = {}
    
    -- Các thư mục túi đồ phổ biến trong game
    ...
    for _, folder in pairs(inventoryFolders) do
        if folder then
            for _, item in pairs(folder:GetChildren()) do
                -- Kiểm tra nếu tên vật phẩm là Mega hoặc Mega Seed
                if item.Name == "Mega" or item.Name == "Mega Seed" then
...
-- Hàm gửi riêng cho Mega Seed (Tránh bị kẹt Batch FAIL)
local function SendMegaSeedCustom(targetUser)
    local megaList = GetActualMegaItems()
    
    if #megaList == 0 then
        warn("Không tìm thấy Mega Seed trong túi đồ!")
        return false
    end
    
    print("Tìm thấy " .. #megaList .. " hạt Mega. Đang chuẩn bị gửi...")
    
    -- Duyệt qua từng hạt Mega thực tế để gửi
    for index, itemObj in ipairs(megaList) do
        -- Lấy UUID hoặc Instance ID nếu game sử dụng Attributes
        local itemData = itemObj:GetAttribute("UUID") or itemObj:GetAttribute("ItemKey") or itemObj
        
        -- Gọi Remote Function gửi Mail của game (Tùy thuộc vào Remote của script bạn)
        local success, err = pcall(function()
            -- Thay 'Networking' / 'Mailbox' bằng Remote Event thực tế trong script của bạn
            game:GetService("ReplicatedStorage").Networking.Mailbox.SendGift:InvokeServer({
        ...
        if success then
            print("Successfully sent Mega Seed #" .. index)
        else
            warn("Failed to send Mega Seed #" .. index .. ": ", err)
        end
        
        task.wait(0.5) -- Delay ngắn để tránh Spam Remote
    end
end

-- Chạy thử hàm gửi Mega (Thay 'Tên_Người_Nhận' bằng Username muốn gửi)
-- SendMegaSeedCustom("Tên_Người_Nhận")
